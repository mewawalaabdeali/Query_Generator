...
def clean_sql_output(response_text):
    """Removes markdown formatiing and extracts the raw SQL query"""
    #Remove Markdown code block formatting ('''sql ... ''')
    clean_query = re.sub(r"```sql\n(.*?)\n```", r"\1", response_text, flags=re.DOTALL)

    #Extact only valid SQL(handles AI explaination)
...
def suggest_index(sql_query):
    ...
    try:
        ...
        return f"Could not generate an execution plan: {e}"
...
def execute_query(sql_query):
    ...
    try:
        ...
        #Open a new connection for query optmization (avoid "commands out of sync" issue)
